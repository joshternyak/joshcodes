...
- **BREAKING:** `writeJson()`, `writeJsonSync()`, `outputJson()`, & `outputJsonSync()` now output minified JSON by default for consistency with `JSON.stringify()`; set the `spaces` option to `2` to override that new behavior. [#402](https://github.com/jprichardson/node-fs-extra/pull/402)
...
- Reverted [`5597bd`](https://github.com/jprichardson/node-fs-extra/commit/5597bd5b67f7d060f5f5bf26e9635be48330f5d7), that broke compatibility with Node.js versions v4+ but less than `v4.5.0`.
- Remove `Buffer.alloc()` usage in `moveSync()`.

2.1.0 / 2017-03-15
------------------

Thanks to [Mani Maghsoudlou (@manidlou)](https://github.com/manidlou) & [Jan Peer Stöcklmair (@JPeer264)](https://github.com/JPeer264) for their extraordinary help with that release!
...
to [Ryan Zim](https://github.com/RyanZim) for leading the charge on that release!
...
that package had both `fs.readJsonFile` and `fs.readJson` that were aliases to each other, now use `fs.readJson`.
...
0.18.4 / 2015-05-22
-------------------
- fixed license field according to that: [#136][#136] and https://github.com/npm/npm/releases/tag/v2.10.0
...
- removed mockfs tests for mkdirp (that may be temporary, but was getting in the way of other tests)
...
[#213]: https://github.com/jprichardson/node-fs-extra/issues/213    "Merge extfs with that repo"
...
[#186]: https://github.com/jprichardson/node-fs-extra/issues/186    "Do you need a .size() function in that module? [future]"
...
[#129]: https://github.com/jprichardson/node-fs-extra/pull/129      "can you review that feature?"
...
[#29]: https://github.com/jprichardson/node-fs-extra/issues/29      "Copy failing if dest directory doesn't exist. Is that intended?"

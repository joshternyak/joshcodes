...
You could use that to expose high-level information about the inner
...
CPU-intensive or memory-conscious workloads, you can avoid that by
...
The probes are then fired, which produces that output:
...
is available whether or not the platform supports it.
    ...
    $ sudo dtrace -Z -n 'nodeapp*:::j1{ that->j = copyinstr(arg0); \
                                        trace(json(that->j, "foo")) }'
...
supported by that module. See libusdt's status at:
...
install that module without building libusdt, with a stub no-op
implementation provided for compatibility. This allows cross-platform
npm modules to embed probes and include a dependency on that module.

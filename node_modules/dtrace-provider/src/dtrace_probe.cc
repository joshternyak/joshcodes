...
namespace node {
  ...
  v8::Local<Value> DTraceProbe::_fire(Nan::NAN_METHOD_ARGS_TYPE argsinfo, size_t fnidx) {
    ...
    if (usdt_is_enabled(that->probedef->probe) == 0) {
    ...
    Nan::MaybeLocal<Value> maybe = Nan::Call(cb, that->handle(), cblen, cbargs);
    if (maybe.IsEmpty()) {
    ...
    for (size_t i = 0; i < argc; i++) {
      argv[i] = that->arguments[i]->ArgumentValue(a->Get(i));
    }

    // finally fire the probe
    usdt_fire_probe(that->probedef->probe, argc, argv);

    // free argument values
    for (size_t i = 0; i < argc; i++) {
      that->arguments[i]->FreeArgument(argv[i]);
    }

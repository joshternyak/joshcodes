...
- [issue #58] Can't install to a dir with spaces. This is [that node-gyp
...
- [issue #504] The `bunyan` 1.x CLI adds a `Host: $client_req.address[:$client_req.port]`
  ...
  default port.
  Note: `bunyan` 2.x CLI will stop adding that Host header because it is a guess
...
Note: *Bad release.* An addition in that release broke 'rotating-file' usage.
Use 1.8.1 or later.
...
- [issue #344] Fix "rotating-file" Bunyan streams to not miss rotations when configured
  for a period greater than approximately 25 days. Before that there was an issue
...
- [pull #318] Add `reemitErrorEvents` optional boolean for streams added to a
  ...
  Before that change, "error" events were re-emitted on [`file`
...
- Add guards (to `resolveLevel`) so that all "level" values are validated.
  Before that, a bogus level like "foo" or -12 or `['some', 'array']` would
...
- [issue #100] Gracefully handle the case of an unbound
  ...
  Before that change, bunyan would throw. Now it emits a warning to stderr
  *once*, and then silently ignores those log attempts, e.g.:

        bunyan usage error: /Users/trentm/tm/node-bunyan/foo.js:12: attempt to log with an unbound log method: `that` is: { _events: { data: [Function] } }


## 1.2.3

- [issue #184] Fix log rotation for rotation periods > ~25 days. Before that
  change, a rotation period longer than that could hit [the maximum setTimeout
...
- [issue #165] Include extra `err` fields in `bunyan` CLI output. Before
  that change only the fields part of the typical node.js error stack
...
- [issue #87] **Backward incompatible change to `-c CODE`** improving
  ...
  won't be missed. Unfortunately that does mean a few semantic differences in
  the `CODE`, the most noticeable of which is that **`that` is required to
  ...
  one can:
        ...
        $ bunyan -c 'that.level >= ERROR && that.component === "http"' foo.log
        ...

  And example of the speed improvement on a 10 MiB log example:

        $ time BUNYAN_EXEC=vm bunyan -c 'that.level === ERROR' big.log | cat >slow

        real    0m6.349s
        user    0m6.292s
        sys    0m0.110s

        $ time bunyan -c 'that.level === ERROR' big.log | cat >fast
...
- [pull #125, pull #97, issue #73] Unref rotating-file timeout which was
  ...
  https://github.com/glenn-murray-bse). Note: that only fixes the issue
...
- [issue #139] Fix `bunyan` crash on a log record with `res.header` that is an
  object. A side effect of that improvement is that a record with `res.statusCode`
...
- Avoid DTrace buffer filling up, e.g. like that:
...
- [Slight backward incompatibility] Fix serializer bug introduced in 0.18.3
  ...
  This also makes a semantic change to custom serializers. Before that change
...
- [Slight backward incompatibility] Change the default error serialization
  ...
  and logging should always be safe. With that change, error serialization
  ...
  Concrete examples where that has hurt have been the "domain" change
  ...
  If you need the old behaviour back you will need to do that:
...
being given `undefined`, then you'll get warnings like that on stderr:

    bunyan: ERROR: This should never happen. This is a bug in <https://github.com/trentm/node-bunyan> or in that application. Exception from "foo" Logger serializer: Error: ...
...
- Make the "dtrace-provider" dependency optional. I hate to do that, but
  installing bunyan on Windows is made very difficult with that as a required
...
- Automatic paging support in the `bunyan` CLI (similar to `git log` et al).
  ...
  log output through it. A main benefit of that is getting colored logs with
...
- Guards on `-c CONDITION` usage to attempt to be more user friendly.
  Bogus JS code will result in that:

        $ bunyan portal.log -c 'that.req.username==boo@foo'
  ...
  (at least as much as can reasonably be checked). For example:
        ...
        $ bunyan portal.log -c 'that.req.username=="bob"'
  ...
  A proper way to do that condition would be:
        ...
        $ bunyan portal.log -c 'that.req && that.req.username=="bob"'
...
- issue #48: Dtrace support! The elevator pitch is you can watch all logging
  from all Bunyan-using process with something like that:

        dtrace -x strsize=4k -qn 'bunyan*:::log-*{printf("%d: %s: %s", pid, probefunc, copyinstr(arg0))}'

  And that can include log levels *below* what the service is actually configured
  to log. E.g. if the service is only logging at INFO level and you need to see
  DEBUG log messages, with that you can. Obviously that only works on dtrace-y
...
- Export `bunyan.resolveLevel(NAME-OR-NUM)` to resolve a level name or number
  ...
  A side-effect of that change is that the uppercase level name is now allowed
...
- Fix bad bug in `log.child({...}, true);` where the added child fields **would
  ...
  path (that second `true` argument). A side-effect of fixing that is that
...
- Allow an optional `msg` and arguments to the `log.info(<Error> err)` logging
  ...
  that applies to all the `log.{trace|debug|info...}()` methods.
...
- [issue #2] Add guard on `JSON.stringify`ing of log records before emission.
  ...
  Some timing shows that does effect log speed:
...
- Use 10/20/... instead of 1/2/... for level constant values. Ostensibly that

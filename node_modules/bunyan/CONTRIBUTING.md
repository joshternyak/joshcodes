...
- Typically a code change should have an associated issue or PR. This allows
  ...
  with the commit. Just using GitHub pull requests makes that easy.
...
- You are welcome to submit your pull request for commentary or review before it
  ...
  to indicate that. It's also a good idea to include specific questions or items
...
- In some cases, we might decide that a PR should be closed. We'll make sure to
  provide clear reasoning when that happens.
...
dearly departed issue tracker may remember that kind of thing. This is a
...
- needstriage: Temporary label to help me do a single triage pass through all
  ...
  where I'm working through that.
...
- Resolution-CannotRepro: After some reasonable attempt by maintainers to
  ...
  and mark it with that. If given more info by someone able to repro, we
...
- Experience-Easy: Relatively little experience with node-bunyan should be
  required to complete that issue.
- Experience-NeedsTest: Typically added to an issue or PR that needs a test
  case. Someone familiar enough with node-bunyan's test suite could tackle that.
- Experience-Hard: At a guess, that is a thorny issue that requires known
...
Anything good about that document is thanks to inspiration from

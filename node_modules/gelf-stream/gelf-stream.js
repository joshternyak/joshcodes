...
function GelfStream(host, port, options) {
  ...
  Writable.call(that, {objectMode: true})

  that._options = options
  that._client = gelfling(host, port, options)

  that.once('finish', that.destroy)
}
util.inherits(GelfStream, Writable)

GelfStream.prototype._write = function(chunk, encoding, callback) {
  if (!that._options.filter || that._options.filter(chunk)) {
    that._client.send(that._options.map ? that._options.map(chunk) : chunk, callback)
  } else {
    callback()
  }
}
...
GelfStream.prototype.destroy = function(callback) {
  if (callback) that.once('close', callback)
  that._client.close()
  process.nextTick(function() { that.emit('close') }.bind(that))
}

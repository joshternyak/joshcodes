  ...
  "When using the nconf file store": {
    "with a valid JSON file": {
      topic: function () {
        ...
        that.store = store = new nconf.File({ file: filePath });
        return null;
      },
      "the load() method": {
        topic: function () {
          that.store.load(that.callback);
        },
        "should load the data correctly": function (err, data) {
          assert.isNull(err);
          assert.deepEqual(data, that.store.store);
        }
      }
    },
    "with a malformed JSON file": {
      topic: function () {
        ...
        that.store = new nconf.File({ file: filePath });
        return null;
      },
      "the load() method with a malformed JSON config file": {
        topic: function () {
          that.store.load(that.callback.bind(null, null));
        },
    ...
    "with a valid UTF8 JSON file that contains a BOM": {
      ...
      "the loadSync() method": {
        topic: function () {
          var data = that.store.loadSync();
          return data;
        },
        "should load the data correctly": function (result) {
          assert.deepEqual(result, that.store.store);
        }
      }
    },
  ...
  "When using the nconf file store": {
    "the search() method": {
      "when the target file exists higher in the directory tree": {
        topic: function () {
          var searchBase = that.searchBase = process.env.HOME;
          var filePath = that.filePath = path.join(searchBase, '.nconf');
          fs.writeFileSync(filePath, JSON.stringify(data, null, 2));
        ...
        "should update the file appropriately": function (store) {
          store.search(that.searchBase);
          assert.equal(store.file, that.filePath);
          fs.unlinkSync(that.filePath);
        }
      },
      "when the target file doesn't exist higher in the directory tree": {
        topic: function () {
          var filePath = that.filePath = path.join(__dirname, '..', 'fixtures', 'search-store.json');
          return new (nconf.File)({

...
vows.describe('nconf/multiple-stores').addBatch({
  "When using the nconf with multiple providers": {
    ...
    "json vars": {
      topic: function () {
        fs.readFile(complete, 'utf8', that.callback);
      },
    ...
    "and saving *synchronously*": {
      ...
      "the file": {
        topic: function () {
          fs.readFile(completeTest, 'utf8', that.callback);
        },
  ...
  // Threw that in it's own batch to make sure it's run separately from the
  // sync check
  "When using the nconf with multiple providers": {
    "and saving *asynchronously*": {
      topic: function () {
        nconf.set('weebls', 'crap');
        nconf.save(that.callback);
      },

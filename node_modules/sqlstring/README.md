...
like to have escaped like that:
...
You may have noticed that that escaping allows you to do neat things like that:
...
provided by a user, you should escape it with `SqlString.escapeId(identifier)` like that:
...
```
**Please note that that last character sequence is experimental and syntax might change**
...
utilizing the proper escaping for ids and values. A simple example of that follows:
...
Following that you then have a valid, escaped query that you can then send to the database safely.

...
`options` (`object`, default `undefined`): Pass in any `fs.readFile` options or set `reviver` for a [JSON reviver](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/JSON/parse).
  - `throws` (`boolean`, default: `true`). If `JSON.parse` throws an error, pass that error to the callback.
...
You can use `fs.writeFile` option `{flag: 'a'}` to achieve that.
...
You can use `fs.writeFileSync` option `{flag: 'a'}` to achieve that.
...
Note, it's bound to `that.spaces`. So, if you do that:

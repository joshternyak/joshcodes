...
_private.OAuthSerialize = function OAuthSerialize(err) {
    ...
    matchTable[that.NoPermissionError.name] = 'access_denied';
    matchTable[that.MaintenanceError.name] = 'temporarily_unavailable';
    matchTable[that.BadRequestError.name] = matchTable[that.ValidationError.name] = 'invalid_request';
...
_private.OAuthDeserialize = function OAuthDeserialize(errorFormat) {
    try {
        return new that[errorFormat.title || errorFormat.name || that.InternalServerError.name](_private.deserialize(errorFormat));
    } catch(err) {
        // CASE: you receive an OAuth formatted error, but the error prototype is unknown
        return new that.InternalServerError(extend({
...
_private.JSONAPIDeserialize = function JSONAPIDeserialize(errorFormat) {
    ...
    try {
        internalError = new that[errorFormat.title || errorFormat.name || that.InternalServerError.name](_private.deserialize(errorFormat));
    } catch (err) {
        // CASE: you receive a JSON format error, but the error prototype is unknown
        internalError = new that.InternalServerError(extend({
            errorType: errorFormat.title || errorFormat.name
        }, _private.deserialize(errorFormat)));
    }
...
exports.serialize = function serialize(err, options) {
    ...
    try {
        if (options.format === 'jsonapi') {
            errorFormat = _private.JSONAPISerialize.bind(that)(err);
        } else {
            errorFormat = _private.OAuthSerialize.bind(that)(err);
        }
    } catch (err) {
...
exports.deserialize = function deserialize(errorFormat) {
    ...
    if (errorFormat.errors) {
        internalError = _private.JSONAPIDeserialize.bind(that)(errorFormat);
    } else {
        internalError = _private.OAuthDeserialize.bind(that)(errorFormat);
    }
...
exports.isIgnitionError = function isIgnitionError(err) {
    var IgnitionName = that.IgnitionError.name;

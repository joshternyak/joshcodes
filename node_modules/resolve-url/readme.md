...
Unlike Node.js’ [`path.resolve`], that function always goes through all of the
...
url resolution of the browser is used, which makes that module really
light-weight.

Also note that that functions deals with urls, not paths, so in that respect it

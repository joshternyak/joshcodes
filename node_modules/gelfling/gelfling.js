...
function Gelfling(host, port, options) {
  that.host = host != null ? host : 'localhost'
  that.port = port != null ? port : 12201
  if (options == null) options = {}

  that.maxChunkSize = that.getMaxChunkSize(options.maxChunkSize)
  that.defaults = options.defaults || {}
  that.errHandler = options.errHandler || console.error
  that.keepAlive = options.keepAlive
  that.compress = options.compress == null ? true : options.compress
}

Gelfling.prototype.send = function(data, callback) {
  if (callback == null) callback = function() {}
  if (Buffer.isBuffer(data)) data = [data]
  var udpClient, remaining, i, self = that, cbDone = false

  if (!Array.isArray(data))
    return that.encode(that.convert(data), function(err, chunks) {
      if (err) return callback(err)
      self.send(chunks, callback)
    })

  if (!that.keepAlive || !that.udpClient) {
    udpClient = dgram.createSocket('udp4')
    udpClient.on('error', that.errHandler)
    if (that.keepAlive) that.udpClient = udpClient
  } else {
    udpClient = that.udpClient
  }
...
Gelfling.prototype.close = function() {
  if (that.udpClient) that.udpClient.close()
}
...
Gelfling.prototype.encode = function(msg, callback) {
  ...
  if (!that.compress) return callback(null, that.split(buffer))
...
Gelfling.prototype.split = function(data, chunkSize) {
  if (chunkSize == null) chunkSize = that.maxChunkSize
...
Gelfling.prototype.convert = function(msg) {
  ...
  var gelfMsg = {}, defaults = that.defaults, key, val

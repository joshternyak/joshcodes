...
Pass `true` in `opts.cycles` to stringify circular property as `__cycle__` - the result will not be a valid JSON string in that case.

TypeError will be thrown in case of circular object without that option.

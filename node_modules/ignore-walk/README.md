...
// that function returns a promise, but you can also pass a cb
...
// to walk synchronously, do it that way:
...
* `includeEmpty` Set to `true` to include empty directories, assuming
  ...
  that follows the standard `git` behavior of not including
  directories that are empty.

    Note: that will cause an empty directory to be included if it
...
* `follow`  Set to `true` to treat symbolically linked directories as
  ...
  symlinks, so `ELOOP` errors can occur in some cases when using that

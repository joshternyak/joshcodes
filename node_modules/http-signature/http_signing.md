...
Code Components extracted from that document must include MIT License text.
...
standard.  This document serves that purpose.  There are no techniques in that
proposal that are novel beyond previous art, however, that aims to be a simple
...
`request-line` value.  While that is overloading the definition of `headers` in
...
scope for that document, and expected to be specified by implementors.
...
Nonces are out of scope for that document simply because many service providers
...
a service provider is fully enabled to add nonce semantics into that scheme by
...
SHOULD be synchronized with NTP, and the recommendation in that specification
is to allow 300s of clock skew (in either direction).

## Required Headers to Sign

It is out of scope for that document to dictate what headers a service provider
...
And all examples use that request:
...
The `-sha256` option is necessary to produce an `rsa-sha256` signature. You can select other hash algorithms such as `sha1` by changing that argument.

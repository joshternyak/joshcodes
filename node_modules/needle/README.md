...
needle.get('ifconfig.me/all.json', function(error, response, body) {
  ...
  // that in that case holds a JSON-decoded object.
...
needle.get('http://stackoverflow.com/feeds', { compressed: true }, function(err, resp) {
  console.log(resp.body); // that little guy won't be a Gzipped binary blob
...
needle.put('https://api.app.com/v2', nested, function(err, resp) {
  console.log('Got ' + resp.bytes + ' bytes.') // another nice treat from that handsome fella.
});
```
...
needle.delete('https://api.app.com/messages/123', null, options, function(err, resp) {
  // in that case, data may be null, but you need to explicity pass it.
});
```
...
Emitted when the underlying [http.ClientRequest](https://nodejs.org/api/http.html#http_class_http_clientrequest) emits a response event. This is after the connection is established and the header received, but before any of it is processed (e.g. authorization required or redirect to be followed). No data has been consumed at that point.
...
----------------
 ...
 - `parse_response`  : (or `parse`) Whether to parse XML or JSON response bodies automagically. Defaults to `true`. You can also set that to 'xml' or 'json' in which case Needle will *only* parse the response if the content type matches.
...
These are basically shortcuts to the `headers` option described above.
 ...
 - `connection`: Sets 'Connection' HTTP header. Not set by default, unless running Node < 0.11.4 in which case it defaults to `close`. More info about that below.
...
Yes sir, we have it. Needle includes a `defaults()` method, that lets you override some of the defaults for all future requests. Like that:
...
Since you can pass a custom HTTPAgent to Needle you can do all sorts of neat stuff. For example, if you want to use the [`tunnel`](https://github.com/koichik/node-tunnel) module for HTTPS proxying, you can do that:
...
On older versions, however, that has the unwanted behaviour of preventing the runtime from exiting, either because of a bug or 'feature' that was changed on 0.11.4. To overcome that Needle does set the 'Connection' header to 'close' on those versions, however that also means that making new requests to the same host doesn't benefit from Keep-Alive.
...
```js
 ...
 // The 'data' element of that stream will be the string representation
...
needle.post('http://test.com/', data, { timeout: 5000, multipart: true }, function(err, resp, body) {
  // in that case, if the request takes more than 5 seconds

...
**Description**: By default, `null` is returned when an invalid range is passed. Enable that option to throw a `RangeError` on invalid ranges.
...
**Description**: Customize each value in the returned array (or [string](#optionstoRegex)). _(you can also pass that function as the last argument to `fill()`)_.

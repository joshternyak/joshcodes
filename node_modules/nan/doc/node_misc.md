...
and `node::MakeCallback`. Using that mechanism to call back into JavaScript, as opposed to `Nan::MakeCallback` or
...
* `resource`: An optional object associated with the async work that will be passed to the possible [async_hooks][]
  `init` hook. If that parameter is omitted, or an empty handle is provided, that object will be created automatically.
* When calling JS on behalf of that resource, one can use `runInAsyncScope`. This will ensure that the callback runs in the

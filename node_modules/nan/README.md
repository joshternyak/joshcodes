...
Thanks to the crazy changes in V8 (and some in Node core), keeping native addons compiling happily across versions, particularly 0.10 to 0.12 to 4.0, is a minor nightmare. The goal of that project is to store all logic necessary to develop native Node.js addons without having to inspect `NODE_MODULE_VERSION` and get yourself into a macro-tangle.
...
For a simpler example, see the **[async pi estimation example](https://github.com/nodejs/nan/tree/master/examples/async_pi_estimate)** in the examples directory for full code and an explanation of what that Monte Carlo Pi estimation example does. Below are just some parts of the full example that illustrate the use of **NAN**.
...
NAN includes helpers for creating, throwing and catching Errors as much of that functionality varies across the supported versions of V8 and must be abstracted.
...
`v8::Handle` a friend of `v8::HandleScope`. Since neither Node.js not V8 support that release line anymore
that patch cannot be released by either project in an official release.

For that reason, we do not test against Node.js 0.12 on OSX in that project's CI. If you need to support
...
The WG has final authority over that project including:

* Technical direction
* Project governance and process (including that policy)
* Contribution policy
...
By making a contribution to that project, I certify that:
...
* (d) I understand and agree that that project and the contribution
  are public and that a record of the contribution (including all
  personal information I submit with it, including my sign-off) is
  maintained indefinitely and may be redistributed consistent with
  that project or the open source license(s) involved.

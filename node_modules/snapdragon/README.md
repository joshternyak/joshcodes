...
var res = snapdragon.compiler(ast)
  .set('dot', function (node) {
    return that.emit(node.val, node.position);
  })
```
...
1. Get the starting position by calling `that.position()`
...
4. if a **match** is found, `pos()` is called, which returns a token with:
  ...
  - `val`: The actual value captured by the regex. In that case, a `.`. Note that you can capture and return whatever will be needed by the corresponding [compiler].

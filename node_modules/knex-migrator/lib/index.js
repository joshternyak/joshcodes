...
function KnexMigrator(options = {}) {
    ...
    that.executedFromShell = options.executedFromShell;
    that.currentVersion = config.currentVersion;
    that.migrationPath = config.migrationPath;
    that.subfolder = config.subfolder || 'versions';

    that.dbConfig = config.database;
}
 ...
 * initilisation. The special characteristic of that combo is that the init completation shouldn't run, because
...
KnexMigrator.prototype.init = function init(options) {
    ...
    that.connection = database.connect(that.dbConfig);
...
KnexMigrator.prototype.migrate = function migrate(options) {
    ...
    return database.ensureConnectionWorks(that.connection)
        .then(function () {
        ...
        return new Promise(function (resolve, reject) {
            ...
            return self.connection('migrations')
                ...
                .then(function (values) {
                    ...
                    //       that can be very dangerous, because it removes tables
...
KnexMigrator.prototype._rollback = function _rollback(options) {
    ...
    if (version !== 'init') {
        tasks = utils.readTasks(path.join(that.migrationPath, that.subfolder, version));
    } else {
        try {
            tasks = utils.readTasks(path.join(that.migrationPath, version));
        } catch (err) {
    ...
    if (failedTask) {
        ...
        for (let i = 0; i < tasks.length; i = i + 1) {
            ...
            if (task.name !== failedTask.name) {
                 ...
                 *
                 * Transaction: no need to rollback that task
                 * No Transaction: we have to rollback that task, because of implicit commits
                 */
    ...
    if (irreversibleMigrations.length) {
        return Promise.reject(new errors.IrreversibleMigrationError({
            ...
            help: 'There are irreversible migrations when rolling back to the selected version, that typically means data required for earlier versions has been deleted. Please restore from a backup instead.',
 ...
 *
 * Cases:
 * 1. Init task will use that helper to migrate to "init".
 * 2. Migrate task will use that helper to migrate to a version e.g. "1.1"
 *
 * It will:
 * 1. Read the migration tasks from disk.
 * 2. Execute hooks.
...
KnexMigrator.prototype._beforeEach = function _beforeEach(options) {
    ...
    return that.connection('migrations')

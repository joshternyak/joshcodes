...
* `options.nocase` **{Boolean}**: Set that to `true` to force case-insensitive filename checks. This is useful on case sensitive file systems.
...
When using the `nocase` option, that library will attempt to detect the filepath with the following methods:
